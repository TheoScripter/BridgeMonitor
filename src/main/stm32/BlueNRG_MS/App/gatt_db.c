...
tBleStatus Acc_Update(AxesRaw_t *x_axes, AxesRaw_t *g_axes, AxesRaw_t *m_axes)
{
  ...
  LSM6DSL_AxesRaw_t accel, gyro;
  get_accel_gyro_buffer(&accel, &gyro);

  HOST_TO_LE_16(buff,    -accel.x);
  HOST_TO_LE_16(buff+2,   accel.y);
  HOST_TO_LE_16(buff+4,  -accel.z);

  HOST_TO_LE_16(buff+6,  gyro.x);
  HOST_TO_LE_16(buff+8,  gyro.y);
  HOST_TO_LE_16(buff+10, gyro.z);

  printf("\r\n\r\n\r\n\r\n\r\n");
  printf("\r\n");
  printf("accel: ");
  printf("%02x%02x %02x%02x %02x%02x", buff[0], buff[1], buff[2], buff[3], buff[4], buff[5]);
  printf("\r\n");
  printf("gyro: ");
  printf("%02x%02x %02x%02x %02x%02x", buff[6], buff[7], buff[8], buff[9], buff[10], buff[11]);
  printf("\r\n");
  printf("\r\n\r\n\r\n\r\n\r\n");
...
tBleStatus BlueMS_Environmental_Update(int32_t press, int16_t temp)
{
  ...
  float temp_value = BSP_TSENSOR_ReadTemp();
  _data.data = temp_value;
  HOST_TO_LE_32(buff, _data.buf);
