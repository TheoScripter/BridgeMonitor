...
void MX_BlueNRG_MS_Init(void)
{
  ...
  // HACK: fix mac adress to F2:E3:F8:4E:0D:33
  // don't forget to also change the gap_init OwnAddressType too in sensor.c
  // when set_connectable() is called
  uint8_t fix_bdaddr[] = {0x33, 0x0D, 0x4E, 0xF8, 0xE3, 0xF2};
  ret = aci_hal_write_config_data(CONFIG_DATA_PUBADDR_OFFSET,CONFIG_DATA_PUBADDR_LEN, fix_bdaddr);
  ret = aci_hal_write_config_data(CONFIG_DATA_RANDOM_ADDRESS,CONFIG_DATA_PUBADDR_LEN, fix_bdaddr);
  if (ret) {
    PRINTF("Setting address failed.\n");
  } else {
    ret = aci_hal_read_config_data(CONFIG_DATA_PUBADDR_OFFSET, BDADDR_SIZE, &bdaddr_len_out, bdaddr);
    PRINTF("Addres set: %x:%x:%x:%x:%x:%x\r\n", fix_bdaddr[0],fix_bdaddr[1],fix_bdaddr[2],fix_bdaddr[3],fix_bdaddr[4],fix_bdaddr[5]);
    PRINTF("Addres retrieved: %x:%x:%x:%x:%x:%x\r\n", bdaddr[0],bdaddr[1],bdaddr[2],bdaddr[3],bdaddr[4],bdaddr[5]);
  }
